...
def test_emergency_exit_scream(strategyAddScream, token, scrToken, ibToken, hToken, chain, whale, vault, strategy, gov, strategist, amount):
    run_emergency_exit_test(token, scrToken, ibToken, hToken, chain, whale, vault, strategy, gov, strategist, amount)

@pytest.mark.parametrize(config.fixtures, config.params, indirect=True)
def test_emergency_exit_ib(strategyAddIB, token, scrToken, ibToken, hToken, chain, whale, vault, strategy, gov, strategist, amount):
    run_emergency_exit_test(token, scrToken, ibToken, hToken, chain, whale, vault, strategy, gov, strategist, amount)

@pytest.mark.parametrize(config.fixtures, config.params, indirect=True)
def test_emergency_exit_hnd(strategyAddHND, token, scrToken, ibToken, hToken, chain, whale, vault, strategy, gov, strategist, amount):
    run_emergency_exit_test(token, scrToken, ibToken, hToken, chain, whale, vault, strategy, gov, strategist, amount)

@pytest.mark.parametrize(config.fixtures, config.params, indirect=True)
def test_emergency_exit_lqdr_hnd(strategyAddLqdrHND, token, scrToken, ibToken, hToken, chain, whale, vault, strategy, gov, strategist, amount):
    run_emergency_exit_test(token, scrToken, ibToken, hToken, chain, whale, vault, strategy, gov, strategist, amount)
